"""
Online Boutique - A multi-agent e-commerce system

This package implements a complete online boutique management system using
a multi-agent architecture with the following components:

- Main Agent: online_boutique_manager (coordinates the entire shopping experience)
- Sub-agents:
  * shipping_service: Handles shipping and delivery management
  * customer_service: Provides customer support and assistance
  * payment_processor: Manages secure payment processing
  * shipping_coordinator: Coordinates order fulfillment and delivery
  * marketing_manager: Handles recommendations and promotions

- MCP Server: boutique_mcp_server (provides data and tools for all agents)

Based on the financial_advisor_management system architecture.
"""

__version__ = "1.0.0"
__author__ = "Online Boutique Team"
